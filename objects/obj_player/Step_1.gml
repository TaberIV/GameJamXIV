...
if (action != HIT_STUN) {
	...
	if ((attack_pressed and (grounded or air_attack_charge)) or attacking) {
		...
		// Starts new attack
		cancel = floor(image_index) >= attack_active_frame[attack_num] and attack_cancelable[attack_num];
		hit = hit and cancel;
		if (attacking and attack_weak_pressed and cancel) {
			if (attack_num != 1 or move_input[0] == image_xscale) {
				attack_num++;
